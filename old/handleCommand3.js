...
export async function handleCommand(message) {
    ...
    \`!chest\` - Displays information about chests.
    ...
    \`!buyexp <amount>\` - Buy EXP for **100 Points per 100 EXP**.
    \`!openchest\` - Open a chest for **${CHEST_PRICE} Points**.
...
**Facebook:** <https://www.facebook.com/bijagaming>`;
    ...
    if (command === 'openchest') {
        ...
        try {
            ...
            await userService.addPointsByNickname(nickname, -CHEST_PRICE);
            ...
            // Recompensa de EXP
            const leveledUp = await userService.addPointsByNickname(nickname, reward.reward);
    ...
    if (command === 'points') {
        ...
        if (!nickname) {
            message.reply('❌ Please register using the form before viewing your Points.');
            return;
        }
        try {
            const points = await userService.getpoints(nickname);
    ...
    if (command === 'ranking') {
        try {
            const topUsers = await userService.getTop5Nicknames();
    ...
    if (command === 'referal') {
        ...
        try {
            await userService.addPointsByNickname(streamerNickname, codeInfo.streamerPoints);
            await userService.addPointsByNickname(nickname, codeInfo.userPoints);
        } catch (error) {
    ...
    if (command === 'addpoints') {
        ...
        try {
            await userService.addPointsByNickname(nickname, pointsToAdd);
